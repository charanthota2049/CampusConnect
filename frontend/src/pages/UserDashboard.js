...
import { QrCode, X, Download, HandHelping, CalendarCheck, Megaphone } from 'lucide-react';
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal qr-modal" onClick={e => e.stopPropagation()}>
                <button className="qr-close-btn" onClick={onClose} aria-label="Close"><X size={20} /></button>
...
function UserDashboard() {
    ...
    const [qrModal, setQrModal] = useState({ open: false, qrCode: null, title: '', subtitle: '' });
        ...
        const load = async () => {
            try {
                const [evRes, notRes, regRes, volAppRes, oppRes] = await Promise.all([
                    ...
                    fetch('http://localhost:3001/api/events/my/registrations',          { credentials: 'include' }),
                    fetch('http://localhost:3001/api/events/my/volunteer-applications', { credentials: 'include' }),
                    fetch('http://localhost:3001/api/events/volunteer/opportunities',   { credentials: 'include' }),
                ]);

                const [evData, notData, regData, volAppData, oppData] = await Promise.all([
                    evRes.json(), notRes.json(), regRes.json(), volAppRes.json(), oppRes.json()
                ]);
                ...
                const map = {};
                apps.forEach(a => { map[String(a.eventId?._id || a.eventId)] = { qrCode: a.qrCode, status: a.status }; });
                setAppliedIds(map);
    ...
    const handleVolunteerApply = async (eventId, eventTitle) => {
        ...
        try {
            ...
            if (!res.ok) {
                if (data.qrCode) {
                    setAppliedIds(prev => ({ ...prev, [eventId]: { qrCode: data.qrCode, status: data.status } }));
                    setQrModal({ open: true, qrCode: data.qrCode, title: eventTitle, subtitle: 'Volunteer Pass' });
                }
                return;
            }

            setOpportunities(prev =>
                prev.map(e => (e._id === eventId) ? { ...e, volunteersRemaining: e.volunteersRemaining - 1 } : e)
    ...
    return (
        <div className="dashboard-page">
            ...
            {qrModal.open && (
                <QRModal
                    qrCode={qrModal.qrCode} title={qrModal.title} subtitle={qrModal.subtitle}
                    onClose={() => setQrModal({ open: false, qrCode: null, title: '', subtitle: '' })}
                />
            )}
            ...
            <div className="dashboard-tabs">
                ...
                >
                    <HandHelping size={15} style={{ marginRight: 5 }} />
                    Volunteer
                ...
                <>
                    <div className="dashboard-grid">
                        <div className="stat-card">
                            <div className="stat-icon"><CalendarCheck size={26} /></div>
                        ...
                        <div className="stat-card">
                            <div className="stat-icon"><Megaphone size={26} /></div>
                            <div className="stat-content">
                    ...
                    <div className="dashboard-grid wide">
                        <div className="dashboard-section">
                            ...
                            <ul className="section-list">
                                {events.length > 0
                                    ? events.map(e => <li key={e._id}><strong>{e.title}</strong> — {formatDate(e.date)}</li>)
                        ...
                        <div className="dashboard-section">
                            ...
                            <ul className="section-list">
                                {notices.length > 0
                                    ? notices.map(n => <li key={n._id}>{n.title}</li>)
                                        ...
                                        return (
                                            <li key={r._id || ev?._id} className="reg-list-item">
                                                <div style={{ display:'flex', flexDirection:'column', gap:2 }}>
                                                    ...
                                                    {ev?.date && <span style={{ fontSize:11, color:'#94a3b8' }}>{formatDate(ev.date)}</span>}
                                                </div>
                                                <button
                                                    className="qr-inline-btn"
                                                    onClick={() => setQrModal({ open: true, qrCode: r.qrCode, title, subtitle: 'Your Ticket' })}
                                                >
                    ...
                    <div className="dashboard-grid wide">
                        ...
                        <div className="dashboard-section vol-opportunities">
                            ...
                            {opportunities.length === 0 ? (
                                <div className="empty-state"><p>No volunteer slots are open right now.</p></div>
                            ) : (
                                        ...
                                        return (
                                            <div key={ev._id} className="vol-opportunity-card">
                                                <div className="vol-opp-info">
                                                    ...
                                                    <span className="vol-opp-date">{formatDate(ev.date)} · {ev.location}</span>
                                                    <span className="vol-slots">
                                                        {isFull ? 'Full' : `${ev.volunteersRemaining} slot${ev.volunteersRemaining !== 1 ? 's' : ''} left`}
                                                    </span>
                                                </div>
                                                {applied ? (
                                                    <div className="vol-applied-row">
                                                        ...
                                                        {applied.qrCode && (
                                                            <button
                                                                className="qr-inline-btn"
                                                                onClick={() => setQrModal({ open: true, qrCode: applied.qrCode, title: ev.title, subtitle: 'Volunteer Pass' })}
                                                            >
                                                                <QrCode size={13} /> QR
                                                            </button>
                                                        )}
                        ...
                        <div className="dashboard-section">
                            ...
                            {myApplications.length === 0 ? (
                                <div className="empty-state"><p>You have not applied for any volunteer roles yet.</p></div>
                            ) : (
                                <ul className="section-list">
                                        ...
                                        <li key={app._id} className="vol-app-item">
                                            ...
                                            <div className="vol-app-actions">
                                                ...
                                                {app.qrCode && (
                                                    <button
                                                        className="qr-inline-btn"
                                                        onClick={() => setQrModal({ open: true, qrCode: app.qrCode, title: app.eventId?.title || 'Event', subtitle: 'Volunteer Pass' })}
                                                    >
                                                        <QrCode size={13} /> QR
                                                    </button>
                                                )}
