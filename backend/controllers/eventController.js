...
function canManageEvent(user, event) {
  ...
  if (user.role === "organizer" && String(event.createdBy) === String(user.id))
    return true;
  return false;
}
...
export async function registerForEvent(req, res) {
  try {
    ...
    if (existing)
      return res
        .status(400)
        .json({ message: "Already registered", qrCode: existing.qrCode });
...
export async function applyAsVolunteer(req, res) {
  try {
    ...
    if (event.maxVolunteers === 0)
      return res
        .status(400)
        .json({ message: "This event does not accept volunteers" });
    if (event.volunteersRemaining <= 0)
...
export async function getMyVolunteerApplications(req, res) {
  try {
    ...
    const applications = await VolunteerApplication.find({ userId })
      .populate(
        "eventId",
        "title date location maxVolunteers volunteersRemaining",
      )
      .sort({ createdAt: -1 });
...
export async function getEventVolunteerApplications(req, res) {
  try {
    ...
    const applications = await VolunteerApplication.find({ eventId })
      .populate("userId", "name email")
      .sort({ createdAt: -1 });
    res.json({ applications });
  } catch (err) {
  ...
  try {
    ...
    if (status === "approved" && prev === "rejected") {
      ...
      if (freshEvent.volunteersRemaining <= 0) {
        return res
          .status(400)
          .json({ message: "No volunteer slots remaining" });
      }
      await Event.findByIdAndUpdate(eventId, {
        $inc: { volunteersRemaining: -1 },
      });
